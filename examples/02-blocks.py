...
async def main():
    ...
    async def task1(*args):
        logger.info(f'TASK: task1 executing')
        return 'task1 result'

    @flow.function_task
    async def task2(*args):
        logger.info(f'TASK: task2 executing')
        return 'task2 result'

    @flow.function_task
    async def task3(*args):
        logger.info(f'TASK: task3 executing')
        return 'task3 result'
    ...
    results = await asyncio.gather(*[run_blocks(i) for i in range(2)])
