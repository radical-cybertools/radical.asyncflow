...
def _panel_gantt(ax, tl: dict, max_tasks: int = 80):
    ...
    ax.legend(handles=patches, fontsize=7, ncol=4, loc="upper right", framealpha=0.9)
    _decorate(
        ax,
        f"Task Gantt  (first {len(tasks)} tasks)",
        "Time since start (s)",
        "Tasks",
        rotate_x=True,
    )
...
def _panel_concurrency(ax, t_c, y_c):
    ...
    _decorate(
        ax, "Task Concurrency", "Time since start (s)", "Tasks running", rotate_x=True
    )
...
def _panel_waterfall(ax, tl: dict):
    ...
    _decorate(
        ax,
        "Task Waterfall  (top 60 by e2e time)",
        "Total time (ms)",
        "Tasks (sorted by e2e)",
        rotate_x=True,
    )
...
def _panel_dep_wait(ax, tl: dict):
    ...
    if waits:
        ...
        ax.hist(
            waits, bins=nb, color="#74b9ff", edgecolor="white", linewidth=0.6, zorder=3
        )
        mu = np.mean(waits)
        ax.axvline(
            mu, color="#ee5a24", linestyle="--", linewidth=2, label=f"mean {mu:.1f} ms"
        )
        ax.axvline(
            np.median(waits),
            color="#f9ca24",
            linestyle="--",
            linewidth=2,
            label=f"median {np.median(waits):.1f} ms",
        )
...
def _panel_conc_resources(ax, events: list[dict], t0: float, t_c, y_c):
    ...
    if t_c:
        ...
        ax.fill_between(t_ms, y_c, step="post", alpha=0.35, color="#74b9ff", zorder=2)
        ax.step(
            t_ms,
            y_c,
            color="#2e86de",
    ...
    _decorate(
        ax,
        "Concurrency Profile  +  Node Resource Utilization",
