import pytest
import asyncio
import time

from concurrent.futures import Future as SyncFuture

from radical.asyncflow.errors import DependencyFailure
from radical.asyncflow import WorkflowEngine
from radical.asyncflow import ThreadExecutionBackend


class TestDependencyFailurePropagation:
    """Test suite for dependency failure propagation in WorkflowEngine."""

    @pytest.fixture
    def engine(self):
        """Create a WorkflowEngine instance for testing."""
        backend = ThreadExecutionBackend({})
        engine = WorkflowEngine(backend=backend, jupyter_async=False)
        yield engine
        engine.shutdown(skip_execution_backend=True)

    @pytest.fixture
    @pytest.mark.asyncio
    def async_engine(self):
        """Create a WorkflowEngine instance for async testing."""
        backend = ThreadExecutionBackend({})
        engine = WorkflowEngine(backend=backend, jupyter_async=True)
        yield engine
        asyncio.create_task(engine.shutdown(skip_execution_backend=True))

    def test_dependency_failure_exception_creation(self, engine):
        """Test that DependencyFailure exceptions are created correctly."""

        # Create a failing task
        @engine.function_task
        def failing_task():
            raise ValueError("Original task failure")

        # Create a dependent task
        @engine.function_task  
        def dependent_task(dep):
            return f"Result from {dep}"

        # Execute the failing task
        t1 = failing_task()

        # Wait a bit for the task to fail
        time.sleep(0.1)

        # Check that the task failed with the expected exception
        assert t1.exception() is not None
        assert isinstance(t1.exception(), ValueError)
        assert str(t1.exception()) == "Original task failure"

        # Create the dependent task
        t2 = dependent_task(t1)

        # Wait for dependency failure to propagate
        time.sleep(0.2)

        # Check that the dependent task failed with DependencyFailure
        assert t2.exception() is not None
        assert isinstance(t2.exception(), DependencyFailure)

        # Verify the DependencyFailure contains proper information
        dep_failure = t2.exception()
        assert "Cannot execute 'dependent_task' due to dependency failure" in str(dep_failure)
        assert "failing_task" in dep_failure.failed_dependencies
        assert isinstance(dep_failure.root_cause, ValueError)
        assert str(dep_failure.root_cause) == "Original task failure"

    def test_multiple_dependency_failures(self, engine):
        """Test failure propagation when multiple dependencies fail."""

        @engine.function_task
        def failing_task1():
            raise ValueError("Task 1 failed")

        @engine.function_task
        def failing_task2():
            raise RuntimeError("Task 2 failed") 

        @engine.function_task
        def dependent_task(dep1, dep2):
            return f"Results: {dep1}, {dep2}"

        # Execute failing tasks
        t1 = failing_task1()
        t2 = failing_task2()

        # Wait for tasks to fail
        time.sleep(0.1)

        # Create dependent task
        t3 = dependent_task(t1, t2)

        # Wait for dependency failure propagation
        time.sleep(0.2)

        # Verify the dependent task failed with DependencyFailure
        assert t3.exception() is not None
        assert isinstance(t3.exception(), DependencyFailure)

        dep_failure = t3.exception()
        assert len(dep_failure.failed_dependencies) == 2
        assert "failing_task1" in dep_failure.failed_dependencies
        assert "failing_task2" in dep_failure.failed_dependencies

        # Root cause should be the first failure encountered
        assert isinstance(dep_failure.root_cause, (ValueError, RuntimeError))

    def test_chain_of_dependency_failures(self, engine):
        """Test failure propagation through a chain of dependencies."""

        @engine.function_task
        def task1():
            raise ValueError("Root failure")

        @engine.function_task
        def task2(dep):
            return f"Task2 result: {dep}"

        @engine.function_task
        def task3(dep):
            return f"Task3 result: {dep}"

        @engine.function_task
        def task4(dep):
            return f"Task4 result: {dep}"

        # Create the chain: task1 -> task2 -> task3 -> task4
        t1 = task1()
        t2 = task2(t1)
        t3 = task3(t2) 
        t4 = task4(t3)

        # Wait for all failures to propagate
        time.sleep(0.3)

        # Verify all dependent tasks failed with DependencyFailure
        for task_future in [t2, t3, t4]:
            assert task_future.exception() is not None
            assert isinstance(task_future.exception(), DependencyFailure)

        # Verify each has the correct root cause
        for task_future in [t2, t3, t4]:
            dep_failure = task_future.exception()
            # The root cause should eventually trace back to the original ValueError
            root_cause = dep_failure.root_cause
            while isinstance(root_cause, DependencyFailure) and root_cause.root_cause:
                root_cause = root_cause.root_cause
            assert isinstance(root_cause, ValueError)
            assert str(root_cause) == "Root failure"

    def test_partial_dependency_failure(self, engine):
        """Test when only some dependencies fail."""

        @engine.function_task
        def successful_task():
            return "Success!"

        @engine.function_task
        def failing_task():
            raise ValueError("This task failed")

        @engine.function_task
        def dependent_task(good_dep, bad_dep):
            return f"Results: {good_dep}, {bad_dep}"

        # Execute tasks
        t1 = successful_task()
        t2 = failing_task()

        # Wait for completion/failure
        time.sleep(0.1)

        # Verify individual task states
        assert t1.result() == "Success!"
        assert isinstance(t2.exception(), ValueError)

        # Create dependent task
        t3 = dependent_task(t1, t2)

        # Wait for dependency failure propagation
        time.sleep(0.2)
        
        # Verify dependent task failed due to bad dependency
        assert isinstance(t3.exception(), DependencyFailure)
        dep_failure = t3.exception()
        assert "failing_task" in dep_failure.failed_dependencies
        assert "successful_task" not in dep_failure.failed_dependencies

    def test_block_dependency_failure(self, engine):
        """Test dependency failure propagation with blocks."""

        @engine.function_task
        def failing_task():
            raise RuntimeError("Task failure")

        @engine.block
        def dependent_block(dep):
            # This block should never execute due to dependency failure
            return f"Block result: {dep}"

        # Execute failing task
        t1 = failing_task()

        # Wait for task to fail
        time.sleep(0.1)

        # Create dependent block
        b1 = dependent_block(t1)

        # Wait for dependency failure propagation
        time.sleep(0.2)

        # Verify block failed with DependencyFailure
        assert isinstance(b1.exception(), DependencyFailure)
        dep_failure = b1.exception()
        assert isinstance(dep_failure.root_cause, RuntimeError)
        assert str(dep_failure.root_cause) == "Task failure"

    @pytest.mark.asyncio
    async def failed_test_async_dependency_failure_propagation(self, async_engine):
        """Test dependency failure propagation in async context."""

        @async_engine.function_task
        async def async_failing_task():
            await asyncio.sleep(0.01)
            raise ValueError("Async task failed")

        @async_engine.function_task
        async def async_dependent_task(dep):
            await asyncio.sleep(0.01)
            return f"Async result: {dep}"

        # Execute tasks
        t1 = async_failing_task()

        # Wait for failure
        await asyncio.sleep(0.1)

        # Create dependent task
        t2 = async_dependent_task(t1)

        # Wait for dependency failure propagation
        await asyncio.sleep(0.2)

        # Verify failure propagation
        assert isinstance(t2.exception(), DependencyFailure), "Expected DependencyFailure in t2"
        dep_failure = t2.exception()

        # Check that the root cause is a ValueError
        assert isinstance(dep_failure.root_cause, ValueError), "Expected ValueError as root cause"

        # Check that the error message matches
        assert str(dep_failure.root_cause) == "Async task failed", \
            f"Unexpected root cause message: {dep_failure.root_cause}"


    def test_handle_task_failure_with_dependency_failure(self, engine):
        """Test that handle_task_failure properly handles DependencyFailure exceptions."""

        # Create a mock task and future
        mock_task = {
            'uid': 'test.000001',
            'name': 'test_task',
            'exception': ValueError("Original error")
        }

        mock_future = SyncFuture()

        # Create a DependencyFailure to pass as override
        dep_failure = DependencyFailure(
            message="Test dependency failure",
            failed_dependencies=["dep1", "dep2"],
            root_cause=ValueError("Root cause")
        )

        # Add mock task to components
        engine.components[mock_task['uid']] = {
            'future': mock_future,
            'description': {'function': lambda: None}
        }

        # Test passing DependencyFailure as override_error_message
        engine.handle_task_failure(mock_task, mock_future, dep_failure)

        # Verify the future was set with the DependencyFailure exception
        assert mock_future.exception() is dep_failure
        assert isinstance(mock_future.exception(), DependencyFailure)

    def test_exception_chaining_in_dependency_failure(self, engine):
        """Test that exception chaining is properly maintained."""

        @engine.function_task
        def original_failing_task():
            raise ValueError("Original error")

        @engine.function_task
        def dependent_task(dep):
            return f"Result: {dep}"

        # Execute tasks
        t1 = original_failing_task()
        time.sleep(0.1)

        t2 = dependent_task(t1)
        time.sleep(0.2)

        # Verify exception chaining
        dep_failure = t2.exception()
        assert isinstance(dep_failure, DependencyFailure)
        assert dep_failure.__cause__ is not None
        assert isinstance(dep_failure.__cause__, ValueError)
        assert str(dep_failure.__cause__) == "Original error"

    def test_dependency_failure_string_representation(self, engine):
        """Test the string representation of DependencyFailure."""

        root_cause = ValueError("Root error")
        dep_failure = DependencyFailure(
            message="Cannot execute task",
            failed_dependencies=["task1", "task2"],
            root_cause=root_cause
        )

        str_repr = str(dep_failure)
        print(str_repr)
        assert "Cannot execute task" in str(dep_failure)
        assert "task1" in dep_failure.failed_dependencies
        assert "task2" in dep_failure.failed_dependencies

        assert isinstance(dep_failure.root_cause, ValueError)
        assert "Root error" in str(dep_failure.root_cause)