...
- Use implicit `WorkflowEngine` shutdown over explicit one. You can use `try`/`finally` in your async main to ensure `flow.shutdown()` is always called, even on exceptions. However it is **preferred** to use the `WorkflowEngine`'s context manager which automatically executes at exit clean-up procedure such as `flow.shutdown`. 
    - Also, given the fact that more clean-up and start-up procecures may be involved in the engine in future API updates, it is highly recommended to opt for this option. 
- Illustration:
    ```python
    # Explicit version
    async def main():
